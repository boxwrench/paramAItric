...
def build_default_registry() -> WorkflowRegistry:
    ...
    registry.register(
        WorkflowDefinition(
            name="valve_handle",
            intent="Create a valve handle replacement part with customizable socket type (square, hex, round_flat), stem dimensions, and lever geometry. Hub cylinder plus top lever combined on the XY plane, blind socket cavity cut from the base plane via draw_polygon, optional set-screw cross-hole. Used for maintenance/repair scenarios where OEM handles are unavailable.",
            stages=(
                "new_design",
                "verify_clean_state",
                "create_hub_sketch",
