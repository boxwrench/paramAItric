...
def test_smoke_script_routes_four_hole_mounting_plate_workflow(monkeypatch) -> None:
    output_path = Path.cwd() / "manual_test_output" / "smoke_four_hole_mounting_plate_test.stl"
    monkeypatch.setattr(smoke_test.Path, "mkdir", lambda self, parents=False, exist_ok=False: None)
    ...
    def fake_send(base_url: str, command: str, arguments: dict) -> dict:
        recorded_commands.append((command, arguments))
        if command == "new_design":
            return {"ok": True, "result": {"design_name": "Fusion Live Four-Hole Mounting Plate Smoke Test"}}
        if command == "get_scene_info" and arguments.get("workflow_stage") == "verify_clean_state":
            return {"ok": True, "result": {"design_name": "Fusion Live Four-Hole Mounting Plate Smoke Test", "sketches": [], "bodies": [], "exports": []}}
        if command == "create_sketch":
            return {"ok": True, "result": {"sketch": {"token": "sketch-1", "name": "Four-Hole Mounting Plate Smoke Sketch", "plane": "xy"}}}
        if command == "draw_rectangle":
            return {"ok": True, "result": {"sketch_token": "sketch-1", "rectangle_index": 0, "width_cm": 4.0, "height_cm": 3.0}}
        if command == "draw_circle":
        ...
        if command == "extrude_profile":
            return {
                ...
                "result": {
                    "body": {
                        "token": "body-1",
                        "name": "Smoke Four-Hole Mounting Plate",
                        "width_cm": 4.0,
                        "height_cm": 3.0,
                        "thickness_cm": 0.4,
                    }
        ...
        if command == "get_scene_info" and arguments.get("workflow_stage") == "verify_geometry":
            return {
                ...
                "result": {
                    ...
                    "sketches": [{"token": "sketch-1", "name": "Four-Hole Mounting Plate Smoke Sketch", "plane": "xy"}],
                    "bodies": [{"token": "body-1", "name": "Smoke Four-Hole Mounting Plate", "width_cm": 4.0, "height_cm": 3.0, "thickness_cm": 0.4}],
                    "exports": [],
                },
            }
        if command == "export_stl":
    ...
    assert draw_circle_commands[0]["center_x_cm"] == 0.6
    assert draw_circle_commands[0]["center_y_cm"] == 0.7
    assert draw_circle_commands[-1]["center_x_cm"] == 3.4
