#!/usr/bin/env python3
"""Rebuild plates.py from original server.py using AST for accurate extraction."""
...
def extract_method_ast(source_code: str, method_name: str) -> str | None:
    """Extract a method using AST for accurate boundary detection."""
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.FunctionDef) and node.name == method_name:
            ...
            end_line = node.end_lineno  # AST gives 1-indexed end line

            # Extract lines
            method_lines = lines[start_line:end_line]

            # Dedent by removing 4 spaces (class indentation)
            result = []
...
def main() -> int:
    ...
    # Header with class definition and public methods
...
'''
    ...
    # Methods to extract (unique list)
    methods_to_extract = [
    ...
    for method_name in methods_to_extract:
        code = extract_method_ast(source_code, method_name)
        if code:
            methods_code.append(code)
            methods_code.append("")
        else:
            print(f"Warning: Could not extract {method_name}")
